...
//  private String smtpServer="smtp.nwnu.edu.cn"; 
...
//   */
	  /*
	  Message msg=new Message("发送者邮箱",   //发送者的邮件地址
            "接受者邮箱",  //接收者的邮件地址
           "hello",  //邮件标题
           "hi,I miss you very much."); //邮件正文
           */
  ...
  public void sendMail(Message msg){
    ...
    try{
      ...
      sendAndReceive(null,br,pw); //仅仅是为了接收服务器的响应数据, 跳过if判断打印client发送内容
  ...
  private void sendAndReceive(String str,BufferedReader br,PrintWriter pw) throws IOException{
    if (str != null){
      System.out.println("Client>"+str);//本地显示发送内容
      pw.println(str);  //发送完str字符串后，还会发送“\r\n”。发送给SMTP服务器，若注释掉这句通信并未发生
    }
    String response;
